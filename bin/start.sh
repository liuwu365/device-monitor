...
java -Dfile.encoding=UTF-8 -Dyt.env=${ENVIRONMENT} ${GCARGS} -XX:+UseG1GC -XX:+PrintGCDetails -Xloggc:${GC_LOG_DIR}/gc.log.${nowday} -XX:+PrintGCDateStamps -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=logs/device-monitor.dump -classpath ${CLASSPATH} com.device.manage.StartApplication > logs/console.log 2>&1 &
